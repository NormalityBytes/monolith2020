...
import com.spirittesting.academy.config.KafkaProperties;
...
class Monolith2020KafkaResourceIT {
    ...
    void setup() {
        ...
        Monolith2020KafkaResource kafkaResource = new Monolith2020KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/monolith-2020-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/monolith-2020-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
