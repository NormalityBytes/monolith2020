...
export class PersonService {
  ...
  protected convertDateFromClient(person: IPerson): IPerson {
    const copy: IPerson = Object.assign({}, person, {
      geburtstag: person.geburtstag && person.geburtstag.isValid() ? person.geburtstag.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.geburtstag = res.body.geburtstag ? moment(res.body.geburtstag) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((person: IPerson) => {
        person.geburtstag = person.geburtstag ? moment(person.geburtstag) : undefined;
      });
    }
    return res;
  }
