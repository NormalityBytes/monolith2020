...
public class PersonResource {
    ...
    public ResponseEntity<PersonDTO> createPerson(@Valid @RequestBody PersonDTO personDTO) throws URISyntaxException {
        ...
        if (Objects.isNull(personDTO.getUnternehmenId())) {
    ...
    public ResponseEntity<List<PersonDTO>> getAllPeople(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("berater-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where berater is null");
            return new ResponseEntity<>(personService.findAllWhereBeraterIsNull(),
                    HttpStatus.OK);
        }
        if ("kandidat-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where kandidat is null");
            return new ResponseEntity<>(personService.findAllWhereKandidatIsNull(),
                    HttpStatus.OK);
        }
        if ("kontakt-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where kontakt is null");
            return new ResponseEntity<>(personService.findAllWhereKontaktIsNull(),
                    HttpStatus.OK);
        }
